...
use \GoogleUrl\GoogleDOM, GoogleUrl\ProxyDefinition;
/**
...
class GoogleUrl {
    ...
    public function init() {
        ...
        $this->googleParams = array("q" => "", // Search Query
        "start" => 0, // First result number
        "num" => 10, // Number of results per pages
        "complete" => 0, // Suggestion auto
        "pws" => 0, // Personnal search
    ...
    private function setParam($name, $value) {
        ...
        else $this->googleParams[$name] = $value;
    ...
    public function search($searchTerm = null, \GoogleUrl\SimpleProxyInterface $proxy = null) {
         ...
         * CHANGE SEARCH IF NEEDED
         ========================*/
        if (null !== $searchTerm) $this->searchTerm($searchTerm);
        else if (!strlen($this->param("q")) > 0) throw new Exception("Nothing to Search");
        /**=========
         * INIT CURL
         =========*/
        ...
        if ($doc->isCaptcha()) throw new \GoogleUrl\Exception\CaptachaException();
