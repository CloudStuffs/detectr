...
class GoogleDOM extends \DOMDocument {
    ...
    public function getPositions() {
        ...
        foreach ($naturals as $node) {
            ...
            $aTag = $aTag->item(0); /* @var $aTag \DOMElement */
            
            if (!$aTag) continue;
            
            $vars = $aTag->attributes->item(1)->nodeValue;
            ...
            if (($protPos = strpos($url, "://")) > 0) { //if no protocole it means the result is a an relative path to google. then it means than it is not a true natural result
                ...
                $store = new GooglePosition($this->search, $shortUrl, $this->date, $truePosition, $url, $title, $node->C14N());
                $store->setVars($vars);
                $positions[] = $store;
